...
def score_in_training_set(prompt_task_explain, Saving_path, query_time_limit, pg_row_num, pg_column_num, iteration_num, model_name_testLLM):
  ...
  for index_query_times in range(query_time_limit): # The upper limit of calling LLMs
    ...
    if success_failure == 'response in the wrong format':
      ...
      pg_dict_returned, collision_check, env_act_feedback = action_from_response(pg_dict, original_response_dict)
      if collision_check:
        print('Collision!')
      ...
      boxes_list = [item for items in pg_dict.values() for item in items if item.startswith('box')]
      boxes_list_returned = [item for items in pg_dict_returned.values() for item in items if item.startswith('box')]
      if Counter(boxes_list) == Counter(boxes_list_returned):
  ...
  for pg_row_num, pg_column_num in [(2, 2), (2, 4), (4, 4), (4, 8)]:
    ...
    for iteration_num in range(4):
      ...
      pg_dict_initial, remaining_box_dict, success_failure, index_query_times, feedback_to_promptLLM, error_string\
        = score_in_training_set(prompt_task_explain, Testing_path, query_time_limit, pg_row_num, pg_column_num, iteration_num, model_name_testLLM)
