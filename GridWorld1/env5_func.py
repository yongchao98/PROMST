...
def score_in_training_set(prompt_task_explain, Saving_path, query_time_limit, pg_row_num, pg_column_num, goal_num, obs_num, iteration_num, model_name_testLLM, order_required = False):
  ...
  for index_query_times in range(query_time_limit): # The upper limit of calling LLMs
    ...
    if success_failure == 'response in the wrong format':
      ...
      env_act_feedback_list.append(env_act_feedback)  # The record list of env act feedbacks
      #pg_state_list.append(pg_dict)
      if collision_check:
        print('Collision!')
  ...
  return pg_dict_initial, remaining_box_dict, success_failure, index_query_times, feedback_to_promptLLM, error_string
...
def total_score_in_training_set(prompt_task_explain, Testing_path, Saving_path_result, prompt_num, model_name_testLLM):
  ...
  for pg_row_num, pg_column_num, goal_num, obs_num in [(4,4,3,4), (5,5,4,6), (6,6,5,10), (7,7,6,14)]:
    ...
    for iteration_num in range(4):
      ...
      pg_dict_initial, remaining_box_dict, success_failure, index_query_times, feedback_to_promptLLM, error_string\
        = score_in_training_set(prompt_task_explain, Testing_path, query_time_limit, pg_row_num, pg_column_num, goal_num, obs_num, iteration_num, model_name_testLLM, order_required = False)
...
def new_prompt_construct_func_PROMST(prompt_trajectory_list, feedback_to_promptLLM_list, success_failure_list, input_error_prompt_token_limit, model_name_promptLLM, with_score_model):
  ...
  if error_string_moveout != '':
    prompt_to_error_summary = prompt_to_error_summarizer_PROMST(prompt_task_explain, error_string_moveout)
    messages = message_construct_func([prompt_to_error_summary], [])  # message construction
  ...
  if error_string_wrong_order != '':
    prompt_to_error_summary = prompt_to_error_summarizer_PROMST(prompt_task_explain, error_string_wrong_order)
    messages = message_construct_func([prompt_to_error_summary], [])  # message construction
...
def new_prompt_construct_func_APE(prompt_trajectory_list, success_failure_list, model_name_promptLLM):
  ...
  if 'failure over query time limit' in success_failure_list or 'response in the wrong format' in success_failure_list or 'Stuck in the local loop.' in success_failure_list\
          or 'collision' in success_failure_list or 'Move out of the grid' in success_failure_list or 'Wrong picking up order' in success_failure_list:
