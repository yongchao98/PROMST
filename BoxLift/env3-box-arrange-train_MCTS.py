...
parser.add_argument('-Training_path', '--Training_path', default='../BoxLift/train_set/') # The path to the training set
parser.add_argument('-Testing_path', '--Testing_path', default='../BoxLift/test_set/') # The path to the testing set
parser.add_argument('-base_path', '--base_path', default='../BoxLift/') # The base path for the training set
...
if not os.path.exists(Testing_path) or not os.path.exists(Training_path):
    create_env3(Testing_path, repeat_num = 4)
    create_env3(Training_path, repeat_num = 4)
...
Starting_prompt_task_explain = f'''
  ...
  Note that the agents can only lift one box at a time. Each lifting agent can be used only once in each step! You can combine multiple agents to lift one box like "box[3.0V]":"agent[1.5W], agent[2.5W]"! Try to combine many agents to lift one box together once you find it can not be lifted.
  [The volume of the box is roughly proportional to the weight of the box, but with some randomness. Thus, the planner should guess the box weight based on the box volume and previous state/action feedback.]

  Specify your action plan in the JSON format: {{"box[1.7V]":"agent[1.5W]", "box[3.0V]":"agent[1.5W], agent[2.5W], agent[5.5W]"}}. Include a box only if it has lifting agents to lift it next.
...
tree = Tree(base_path=save_path, Testing_path = Testing_path, n_children=8, n_selected=2,
