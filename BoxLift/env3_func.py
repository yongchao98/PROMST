...
def score_in_training_set(prompt_task_explain, Testing_path, query_time_limit, pg_row_num, iteration_num, model_name_testLLM):

  with open(Testing_path+f'/env_pg_state_{pg_row_num}/pg_state{iteration_num}/lifter_weight_list{iteration_num}.txt', 'r') as file:
    lifter_weight_list = [float(line.strip()) for line in file.readlines()]
  file.close()
  with open(Testing_path+f'/env_pg_state_{pg_row_num}/pg_state{iteration_num}/volume_list{iteration_num}.txt', 'r') as file:
    volume_list = [float(line.strip()) for line in file.readlines()]
  file.close()
  with open(Testing_path+f'/env_pg_state_{pg_row_num}/pg_state{iteration_num}/weight_list{iteration_num}.txt', 'r') as file:
  ...
  left_box_list = [] # The record list of left boxes
  ...
  for index_query_times in range(query_time_limit): # The upper limit of calling LLMs
    user_prompt_1 = input_prompt_1_func_total(prompt_task_explain, state_update_prompt, response_total_list,
                              left_box_list, env_act_feedback_list)
    ...
    try:
      ...
      original_response_dict = copy.deepcopy(data)
    ...
    if success_failure == 'response in the wrong format':
      env_act_feedback_list.append('') # The feedback of env act
      ...
      if pg_dict == pg_dict_returned:
        dict_not_update_rounds += 1
      else:
        dict_not_update_rounds = 0
      if dict_not_update_rounds > 4:
        success_failure = 'Stuck in the local loop.'
        system_error_feedback_2 = 'It seems the LLM is stuck in the current situation, always repeating the same answer. The task is stuck too, no box is lifted successfully in recent rounds. The LLM should learn to combine multiple agents (more than two or three) to lift one box together and add the agent number if cannot success multiple times. Do not try to lift all the boxes if the agents are not enough.'
  ...
  # Find the overlap between the two dictionaries
  remaining_box_dict = pg_dict_end
  ...
  # Find the items left in pg_dict_initial after removing the overlap
  lifted_box_dict = {k: pg_dict_initial[k] for k in pg_dict_initial if k not in remaining_box_dict}

  print("Lifted_box_dict:", lifted_box_dict)
  print("Remaining_box_dict:", remaining_box_dict)
  print('\n')

  return lifted_box_dict, remaining_box_dict, success_failure, index_query_times, feedback_to_promptLLM, error_string
...
def total_score_in_training_set(prompt_task_explain, Testing_path, Saving_path_result, prompt_num, model_name_testLLM):
  ...
  #for pg_row_num in [4]:
    ...
    #for iteration_num in range(1):
      ...
      lifted_box_dict, remaining_box_dict, success_failure, index_query_times, feedback_to_promptLLM, error_string\
        = score_in_training_set(prompt_task_explain, Testing_path, query_time_limit, pg_row_num, iteration_num, model_name_testLLM)

      lifted_weight_ratio = np.sum(list(lifted_box_dict.values())) / (
                np.sum(list(lifted_box_dict.values())) + np.sum(list(remaining_box_dict.values())))
      ...
      with open(Saving_path_result + f'/lifted_box_dict_{pg_row_num}_{iteration_num}.json', 'w') as f:
        json.dump(lifted_box_dict, f)
      f.close()

      with open(Saving_path_result + f'/remaining_box_dict_{pg_row_num}_{iteration_num}.json', 'w') as f:
        json.dump(remaining_box_dict, f)
      f.close()

      with open(Saving_path_result + f'/success_failure_{pg_row_num}_{iteration_num}.txt', 'w') as f:
        f.write(success_failure)
      f.close()

      with open(Saving_path_result + f'/feedback_to_promptLLM_{pg_row_num}_{iteration_num}.txt', 'w') as f:
        f.write(feedback_to_promptLLM)
      f.close()

      with open(Saving_path_result + f'/env_action_times_{pg_row_num}_{iteration_num}.txt', 'w') as f:
