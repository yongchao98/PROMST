...
def score_in_training_set(Testing_path, prompt_task_explain, query_time_limit, index, model_name_testLLM):
  ...
  env = pddlgym.make("PDDLEnvManyblockssmallpiles-v0")
  ...
  for index_query_times in range(query_time_limit): # The upper limit of calling LLMs
    ...
    if start != 'pick' and start != 'put' and start != 'stack' and start != 'unstack':
      success_failure = 'response in the wrong format'
    if 'block' not in initial_response:
      success_failure = 'response in the wrong format'
    else:
    ...
    if success_failure == 'response in the wrong format':
      ...
      obs, task_finish, problem_file, invalid_action, env_act_feedback = action_from_response(Testing_path, env, response, problem_file, domain_file)
      if invalid_action:
...
def new_prompt_construct_func(prompt_trajectory_list, feedback_to_promptLLM_list, success_failure_list, input_error_prompt_token_limit, model_name_promptLLM, with_score_model):
  ...
  if error_string_invalid != '':
    prompt_to_error_summary = prompt_to_error_summarizer(prompt_task_explain, error_string_invalid)
    messages = message_construct_func([prompt_to_error_summary], [])  # message construction
